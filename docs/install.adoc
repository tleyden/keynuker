...
```
$ cd /path/to/keynuker
$ aws iam create-policy --policy-name IAMUserListDeleteKeys --policy-document file://docs/iam_policy.json 
$ aws iam create-user --user-name Keynuker
$ aws iam attach-user-policy --policy-arn arn:aws:iam::{aws_account_id}:policy/IAMUserListDeleteKeys --user-name Keynuker
$ aws iam create-access-key --user-name Keynuker
```
